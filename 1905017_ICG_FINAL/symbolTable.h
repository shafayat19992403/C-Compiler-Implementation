...
public:
    ...
    vector<symbolInfo*> parametersAsSi;
    int offset=0;
    symbolInfo()
    {
        next=NULL;
    ...
    symbolInfo(const symbolInfo &s){
        ...
        this->sizeOfArray=s.sizeOfArray;
...
class scopeTable
{
    ...
    bool insert(symbolInfo si){
        int i=1;
        symbolInfo* temp=new symbolInfo(si);
        int bucketNumber = hashFunction(temp->getName());
        symbolInfo* prev,*curr;
        prev=NULL;
        curr=table[bucketNumber];
        ...
        if(LookUp(temp->getName(),true))
        {
            cout<<"\t"<<temp->getName()<<" "<<"already exisits in the current ScopeTable"<<endl;
                return false;
        }
        else
        {
    ...
    void print()
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            while(!isNull(temp)){
                cout<<"<" <<temp->getName() <<","<<temp->getType()<<">(offset:)"<<temp->offset;
                temp=temp->next;
            }
            delete temp;
            if(table[i]!=NULL)
...
public:
    ...
    bool insert(symbolInfo si){
        bool isInsertable=curScopeTable->insert(si);
        return isInsertable;
    }
